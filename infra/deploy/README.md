...
the two overlap, the code is authoritative — the configuration in
`railway/` is *generated* from `processes.py`, and a test fails if the
committed files drift from it.
...
run instructions only, so it cannot become a second place a credential
lives or leaks.
