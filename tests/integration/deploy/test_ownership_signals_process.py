"""Module 29's daily ownership signals as a deployed process.
...
`tests/unit/ownership_signals/` and `tests/integration/ownership_signals/`.
"""
...
from infra.deploy import ownership_signals, scanner
...
def test_it_resolves_the_universe_with_the_scanners_own_function():
    ...
    assert ownership_signals.resolve_universe_version is scanner.resolve_universe_version
    assert ownership_signals.ScannerSettings is scanner.ScannerSettings
...
def test_it_refuses_to_run_when_no_universe_version_is_named(migrated: Engine):
    ...
    with pytest.raises(ScannerNotReady, match=UNIVERSE_VERSION_ENV_VAR):
        ownership_signals.run_scheduled_ownership_signals(migrated, settings=settings)
...
def test_a_missing_universe_exits_with_the_prerequisite_code(migrated: Engine, monkeypatch):
    ...
    monkeypatch.setattr(ownership_signals, "create_db_engine", lambda **kwargs: migrated)

    assert ownership_signals.main([]) == 2
...
def test_the_entrypoint_refuses_arguments_it_cannot_use():
    ...
    with pytest.raises(UnexpectedArguments):
        ownership_signals.main(["&&", "uvicorn", "infra.deploy.asgi:terminal_app"])
...
def test_the_process_definition_and_the_module_agree():
    ...
    assert PROCESSES["ownership_signals"].command() == "python -m infra.deploy.ownership_signals"
    assert callable(ownership_signals.main)
...
def test_a_named_universe_version_produces_a_healthy_run(migrated: Engine):
    ...
    Module 29's own report is healthy whenever nothing due failed to be
    assessed, which an empty universe trivially satisfies."""
    identifier = _label(migrated, "universe-2026-03-09-module29")
    settings = scanner.ScannerSettings(universe_version=identifier)

    report = ownership_signals.run_scheduled_ownership_signals(migrated, settings=settings)
...
def test_a_named_version_resolves_the_same_way_as_every_other_cron(migrated: Engine):
    """The property the shared function buys, asserted rather than assumed."""
    identifier = _label(migrated, "universe-2026-03-09-module29b")

    with migrated.connect() as connection:
        from_ownership_signals = ownership_signals.resolve_universe_version(
            connection, "universe-2026-03-09-module29b"
        )
        from_scanner = scanner.resolve_universe_version(connection, "universe-2026-03-09-module29b")

    assert str(from_ownership_signals) == identifier
    assert from_ownership_signals == from_scanner
